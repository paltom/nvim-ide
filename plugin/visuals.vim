...
execute "highlight STLWarning guibg=".s:stl_bg." guifg=".s:get_hi_color("WarningMsg", "guifg")
execute "highlight STLPercentBlock guibg=".s:stl_bg." gui=inverse"
...
function! s:stl_file_info()
  ...
  " :help 'statusline'
  let l:flags = "%-3.3(%{&modifiable && !&readonly ? (&modified ? ' \u274b ' : '') : '\U1f512 '}%)"
  let l:trunc = "%<"
  ...
  return s:highlight_stl_part(l:flags, "STLWarning").l:trunc.l:filename
endfunction
...
function! Stl()
  ...
  "let l:stl .= s:stl_sep
  "let l:stl .= s:stl_location()
  "let l:stl .= s:stl_sep
  "let l:stl .= s:stl_win_id()
  return l:stl
endfunction

"set statusline=%{pathshorten(fnamemodify(getcwd(),':~'))}:%#WarningSTL#%-3.3(%{&modifiable&&!&readonly?\(&modified?\"\ \\u274b\":''\):\"\\U1f512\ \"}%)%#StatusLine#%<%{expand('%')==''?'[No\ Name]':pathshorten(expand('%:.:h')).expand('/').expand('%:t')}%=%(%y%q%w%)\ %#PercentBlockSTL#%{VisualPercentage()}%#StatusLine#\ [%{winnr()}]
set statusline=%!Stl()
